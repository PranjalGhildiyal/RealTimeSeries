...
class Connection:
    def __init__(self):
        ...
        if not status:
            ...
            return None
        self.logger= Applogger(logging_location['sqldb'])
        ...
        self.lg.info('Initiating Connection with Database.')
    ...
    def connect(self, sql_username:str='', sql_password:str='', sql_ip:str='', sql_port:str='', sql_database:str='')->bool:
        ...
        self.lg.info("Start for making sql_conn with %s, %s, %s, %s, %s" , sql_username, sql_password, sql_ip, sql_port, sql_database)
        try:
            ...
            self.lg.info("Execution success with engine: %s", self.__engine)
            return self.__connection_status
        except Exception as e:
            self.lg.error("Execution failure")
            self.lg.exception("Exception: " + str(e))
            return self.__connection_status
    ...
    def get_schema(self, table_name:str):
        ...
        try:
            ...
            self.lg.info('Executing query: {}'.format(query))
            columns= pd.read_sql(query, self.__engine)
            schema= list(columns['COLUMN_NAME'].values)
            status= True
            self.lg.info('Succcessfuly exported schema: {}'.format(schema))
            return (status, schema)
        
        except Exception as e:
            self.lg.error('Error while getting schema. Error message: {}'.format(e))
    ...
    def import_data(self, timestamp_column:str, value_column:str, callback):
        ...
        self.lg.info("Downloading data from database. Query:  %s, Engine: %s" , query,self.__engine)
        try:
            ...
            self.lg.info("Data imported from database successfully.")
            for i in data.index:
